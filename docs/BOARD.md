...
## 진행 중
- [Y, 7/18] **우리 계정 제출 1회 + 체인 정합 검증** — 생성·검증 완료, **데이콘 제출만 대기** (사용자, 파일 `~/Downloads/submission_static_y.csv`, **예측 0.302228**). 경과: 맥 8GB CPU 불가 → Kaggle T4 킷 336s 완주(exp-01, 원장 기록). Action 평균 0.4283(팀원 대비 −0.0018, 인코딩差) — 체인 정상 판정. best-of-2 시뮬: 정적↔베이스라인 스위치 이득 샘플 **0개**(최대 우세 0.63 < 손익분기 0.77) → 이종 선택 무익 확정
