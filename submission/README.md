...
`reference/` 에 서버에서 실제로 돌아갔던 파일 일곱 개를 남겼다 — 모델 수프(`soup.py`),
0.1810 을 만든 추론 설정, 오케스트레이션 스크립트, pod 야간 작업. 본체가 정리본이라면
그쪽은 **장부의 숫자와 짝이 맞는 원본**이다. 자세한 것은 [reference/README.md](reference/README.md).

거기에 **잃은 것도 적어두었다** — b10y 계보(누적 15,600~22,800)의 설정과 체크포인트는
pod 삭제와 함께 사라져서, 이 저장소만으로는 그 구간이 재현되지 않는다.
