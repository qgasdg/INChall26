#!/bin/bash
# 학습 — 4단계 이어달리기, 합계 10,800스텝. GPU 1장.
#
# 왜 한 번에 안 돌리고 4단계인가: 실제로 그렇게 학습했기 때문이다. 단계를 넘을 때
# **UNet 가중치만** 물려받고 옵티마이저 상태는 새로 시작하며 학습률 워밍업도 다시 돈다.
# 그래서 10,800스텝을 한 번에 도는 것과 결과가 같지 않다 — 재현하려면 이 순서 그대로여야 한다.
#
#   1단계  300스텝  인코더 동결 · 액션 조건 없음 (백본 적응)
#   2단계 2,100스텝 인코더 해제 + 8비트 Adam + 액션 cross-attention 투입
#   3단계 1,800스텝 2단계와 설정 동일
#   4단계 6,600스텝 2단계와 설정 동일
#
# 소요: RTX PRO 6000 96GB 1장 기준 스텝당 약 10.3초 → 합계 약 31시간 (제한 4일)

# 시드: 킷 train_diffusion.py 가 seed_everything(args.seed) 를 부르고 --seed 기본값이
# 20230211 이다. 우리는 --seed 를 넘긴 적이 없으므로 네 단계 모두 20230211 로 돌았다.
...
echo ">>> 최종 가중치: $(ckpt_at stage4 6600)"   # = 누적 10,800 · 이것이 제출 점수를 낸 모델
